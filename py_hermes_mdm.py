...
class MetadataSnapshot:
    ...
    def collect(self):
        ...
        for tag in mdm.bkt_info:
            ...
            #with open('metadata_snapshot3.txt', 'a') as file:
            ...
            #New Lines: Append data to parquet files
            #Save to parquet files
        self.save_to_parquet()
        print("Type of taget_info", type(self.target_info), "blob", type(self.blob_info),"tag", type(self.tag_info))
    ...
    def save_to_parquet(self):
        ...
        target_df = pd.DataFrame(self.target_info)
        blob_df = pd.DataFrame(self.blob_info)
        tag_df = pd.DataFrame(self.tag_info)

        #Save DataFrames to Parquet files
        blob_df.to_parquet('blob_info.parquet', index=False)
        tag_df.to_parquet('tag_info.parquet', index=False)
        target_df.to_parquet('target_info.parquet', index=False)
